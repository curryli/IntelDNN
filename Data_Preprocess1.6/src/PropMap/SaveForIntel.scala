  ...
  def main(args: Array[String]): Unit = {
   ...
   val startTime = System.currentTimeMillis(); 
    ...
    var FraudData = hc.sql(s"select t1.tfr_dt_tm, t1.trans_at, t1.total_disc_at, "+
      ...
      s"where t1.pdate>=20160704 and t1.pdate<=20160704 ").repartition(1000).persist(StorageLevel.MEMORY_AND_DISK_SER)  //.cache 
    ...
    var AllData = hc.sql(s"select tfr_dt_tm, trans_at, total_disc_at, "+
      ...
      s"substring(acct_ins_id_cd,1,4) as acct_ins_id_cd_BK, substring(acct_ins_id_cd,5,4) as acct_ins_id_cd_RG " +
	    s"from tbl_common_his_trans where pdate>=20160702 and pdate<=20160702 ").repartition(1000).persist(StorageLevel.MEMORY_AND_DISK_SER)// .cache         //.persist(StorageLevel.MEMORY_AND_DISK_SER)//
    ...
    result.rdd.saveAsTextFile("xrli/IntelDNN/20160704_Labeled")
