...
//    val a = transdata.groupBy("settle_tp").agg(count("settle_cycle") as "cnt")
//    a.show()   
//    
//    val b = transdata.groupBy("block_id").agg(count("settle_cycle"))
//    b.show()
    ...
    for(item <- DisperseList){
      val a = transdata.groupBy(item).agg(count("settle_cycle"))
      a.show 
    }
