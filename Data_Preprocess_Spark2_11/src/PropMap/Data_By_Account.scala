...
object Data_By_Account {
  ...
  def main(args: Array[String]): Unit = {
    ...
    var AllData = IntelUtil.get_from_HDFS.get_filled_DF(ss, startdate, enddate).repartition(1000)//.persist(StorageLevel.MEMORY_AND_DISK_SER)// .cache         //.persist(StorageLevel.MEMORY_AND_DISK_SER)//
    AllData.show(5)
    ...
    var FraudData = AllData.join(fraud_join_Data, AllData("sys_tra_no")===fraud_join_Data("sys_tra_no") 
                                               ...
                                               && AllData("pdate")===fraud_join_Data("pdate"), "left_semi") 
    println("FraudData count is " + FraudData.count())
   
    //sys_tra_no, pri_acct_no_conv, mchnt_cd, pdate 
    var Fraud_related_Data = AllData.join(fraud_join_Data, AllData("pri_acct_no_conv")===fraud_join_Data("pri_acct_no_conv"), "left_semi") 
    println("Fraud_related_Data count is " + Fraud_related_Data.count())
