...
object save_labeled_new {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val enddate = "20160901"
    var usedArr_filled = IntelUtil.constUtil.usedArr.map{x => x + "_filled"}
    
    val counterfeit_cards= sc.textFile("xrli/IntelDNN/counterfeit_cards_0708.csv").cache
    val counterfeit_cards_list = counterfeit_cards.collect()
    
    val sample_cards= sc.textFile("xrli/IntelDNN/All_sample_cards_1607").cache 
    ...
    val Alldata_by_cards = sc.textFile("xrli/IntelDNN/Counterfeit/Alldata_by_cards").map{str=>
           var tmparr = str.split(",")         
           tmparr = tmparr.map { x => x.toString()}    
           Row.fromSeq(tmparr.toSeq)
       }
    
    val counterfeit = sc.textFile("xrli/IntelDNN/Counterfeit/counterfeit_filled_1607.csv").map{str=>
           var tmparr = str.split(",")         
           tmparr = tmparr.map { x => x.toString()}    
           Row.fromSeq(tmparr.toSeq)
       }
    ...
    LabeledData.rdd.map(_.mkString(",")).saveAsTextFile("xrli/IntelDNN/Counterfeit/Labeled_All_1607")
