...
object save_sample_cards {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val All_sample_cards = AllData.sample(false, 0.00005, 0).select("pri_acct_no_conv").distinct()//.persist(StorageLevel.MEMORY_AND_DISK_SER) 
    
    All_sample_cards.rdd.map(r=>r.getString(0)).coalesce(1).saveAsTextFile("xrli/IntelDNN/All_sample_cards_1607")
