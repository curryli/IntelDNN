...
object Expand_Indexed_RF {
   ...
   var idx_modelname = "xrli/IntelDNN/CashOut/models/index_Model_1101"
...
//    labeledData.show(5)
   ...
   // val feature_arr = no_idx_arr.++(CatVecArr)    //不加载idx_model时使用
    val feature_arr = no_idx_arr.++(Arr_to_idx)      //加载idx_model时使用
    ...
    labeledData.selectExpr(db_list:_*).show()
