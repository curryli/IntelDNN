...
object Read_FE_GBDT {
   ...
   val not_train_list = Array("retri_ref_no", "Sys_tra_no", "Trans_tm","Settle_dt","fwd_settle_at","rcv_settle_at","cdhd_at","certid","card_no","label")
...
//    new_labeled.write.format("com.databricks.spark.csv").mode(SaveMode.Overwrite).options(saveOptions).save()
   ...
   //////////////////////////////////////////save//////////////////////////////////////// 
    ...
    save_DF.dtypes.foreach(println)
        
    save_DF.rdd.map(_.mkString(",")).saveAsTextFile("xrli/credit/new_FE_idx")
